...
# ========= 0) Lecture des données (chaque fichier TD doit contenir cette partie) ==========
CSV_PATH = "vgsales.csv"   # Modifier le chemin / nom du fichier si nécessaire
df = pd.read_csv(CSV_PATH)

# Normaliser les noms de colonnes (tolérance aux erreurs, peut être conservé)
...
# Forcer en valeurs numériques et supprimer les valeurs manquantes
for c in [target] + cols_regions:
    df[c] = pd.to_numeric(df[c], errors="coerce")
df3 = df[[target] + cols_regions].dropna().copy()

# ========= 1) Moindres carrés : Global_Sales ~ NA+EU+JP+Other ==========
y = df3[target].to_numpy()                    # (n,)
X = df3[cols_regions].to_numpy()              # (n,4)
X = np.c_[np.ones(len(X)), X]                 # Ajouter l’interception (n,5)
...
# ========= 2) Visualisation : réel vs prédit ==========
...
# ========= 3) Régression standardisée (comparaison des influences relatives) ==========
...
# ========= 4) VIF (vérification approximative de la multicolinéarité) ==========
def simple_vif(M, i):
    y_ = M[:, i]
